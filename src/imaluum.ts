...
export class IMaluumSubPage {
  ...
  async selectTimetableOrResultDropDownMenuItem(
    semester: string,
    year: string
  ) {
    const [elemHandles, elemTextContents] =
      await this._extractDropDownMenuItem();
    ...
    if (selected.idx === -1)
      throw new Error(
        `[ERROR] ${capitalize(this.type)} for ${
          selected.fullStr
        } does not exist!`
      );
  ...
  > {
    ...
    const elemHandles = await this.page.$$(
