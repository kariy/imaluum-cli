...
export class CommandExecutor {
  ...
  async loginFromCookie() {
    ...
    if (currentUrl !== homePage)
      throw new Error(
        `Wrong page. Expected ${homePage} but got ${currentUrl}.`
      );
  }
  ...
  private _authenticate({
    options,
  }: TCommandActionParams<CommandEnum.Authenticate>) {
  ...
  async result(...args: any[]) {
    if (this.imaluum?.page == null)
      throw new Error("i-Ma'luum page not running!");
  ...
  async timetable(...args: any[]) {
    ...
    const output = await loader.startAsyncTask(async () => {
      ...
      return await this._timetable(
        timetablePage as IMaluumSubPage,
        timetableArgs
      );
    });
